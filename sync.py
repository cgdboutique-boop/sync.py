...
def get_location_id():
    r = safe_request("GET", f"https://{SHOPIFY_STORE}.myshopify.com/admin/api/{SHOPIFY_API_VERSION}/locations.json",
...
def update_price(variant_id, new_price, sku):
    ...
    print(f"✅ Price updated for SKU {sku} → {new_price:.2f}")
    time.sleep(0.5)
...
def update_inventory(inventory_item_id, location_id, quantity, sku):
    ...
    print(f"✅ Inventory set for SKU {sku} → {quantity}")
    time.sleep(0.5)
...
def main():
    ...
    for idx, product in enumerate(supplier_products, start=1):
        sku = str(product.get("sku"))
        price = product.get("price")
        quantity = product.get("quantity", 0)

        if sku not in shopify_variants:
            print(f"{idx}. SKU {sku} not found in Shopify, skipping.")
            continue
        ...
        if str(variant.get("price")) != str(price):
            ...
            print(f"{idx}. SKU {sku} price already correct ({price})")

        # Inventory update
        if int(variant.get("inventory_quantity", 0)) != int(quantity):
            update_inventory(inventory_item_id, location_id, quantity, sku)
        else:
            print(f"{idx}. SKU {sku} inventory already correct ({quantity})")
