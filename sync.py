...
for base_sku, items in sku_groups.items():  
    ...
    options = [{"name": "Size", "values": list(dict.fromkeys(option_values))}]  # remove duplicate option values  
    ...
    if existing:  
        # Match by SKU as well  
        existing_variant_ids = [v["sku"] for v in existing[0].get("variants", [])]  
        if base_sku in existing_variant_ids:  
            ...
            print(f"🆕 Creating new product for SKU: {base_sku}")  
    ...
    if response.status_code in [200, 201]:  
        print(f"✅ Synced: {title} (SKU: {base_sku})")  
